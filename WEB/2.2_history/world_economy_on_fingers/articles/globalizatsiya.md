...
**Глобализация** — это процесс, при котором страны мира становятся всё сильнее связаны друг с другом через торговлю, транспорт, деньги, интернет, технологии, миграцию, идеи и культуру.

Это значит, что вещи, которые мы покупаем, часто создаются не в одной стране, а сразу в нескольких. Например, дизайн товара может появиться в одной стране, детали — в другой, сборка — в третьей, а продажа — в четвёртой. Поэтому глобализация — это одна из самых важных тем для понимания мировой экономики.
...
|---|---|
| Торговля | страны покупают и продают товары друг другу |
| Транспорт | контейнеровозы, самолёты, поезда и грузовики связывают рынки |
| Финансы | деньги, кредиты, инвестиции и валютные расчёты двигаются между странами |
| Технологии | интернет, программирование, облачные сервисы и чипы работают сразу на мировой рынок |
| Культура | фильмы, музыка, игры, мода и соцсети быстро распространяются по миру |

> [!NOTE]
> **Важно:** глобализация — это не то же самое, что «все стали одинаковыми». Скорее это история о том, что страны стали **сильнее зависеть друг от друга**.

Ещё одно важное слово здесь — **цепочка поставок**. Это путь товара от сырья до покупателя.

```mermaid
flowchart LR
    A[Идея и дизайн] --> B[Детали и сырьё]
    B --> C[Сборка]
    C --> D[Морская или воздушная перевозка]
    D --> E[Склад и магазин]
    E --> F[Покупатель]
    G[Банки и платежи] --> E
    H[Интернет и реклама] --> F
```

Такая схема хорошо показывает, что современный товар — это часто результат работы **многих стран сразу**.
...
| Период | Что происходило | Почему это важно |
|---|---|---|
| XIX — начало XX века | пароходы, железные дороги, телеграф | мир начал быстрее обмениваться товарами и новостями |
| После Второй мировой войны | восстановление торговли, новые международные правила, рост роли США | мировая экономика стала более связанной |
| 1960-е и дальше | контейнерные перевозки | стало дешевле и быстрее возить товары между континентами |
| 1990-е | интернет, компьютеры, ускорение мировой торговли | глобализация резко ускорилась |
...
> [!TIP]
> Один из ключевых поворотов — **контейнеризация**. Когда грузы стали возить в стандартных контейнерах, мировая торговля ускорилась: товар стало проще перегружать из корабля в поезд и в грузовик.

Ещё один важный шаг — развитие **глобальных цепочек создания стоимости**. Это когда товар не «делают целиком» в одной стране, а создают по частям: где-то добывают сырьё, где-то делают детали, где-то собирают, а где-то продают и рекламируют.

<a id="map"></a>
## Карта мировой взаимосвязи

Ниже — учебная карта с важнейшими маршрутами, узкими местами и торговыми узлами.

![Глобализация: ключевые морские пути, каналы, проливы и торговые узлы](../images/globalizatsiya_map.png)

*Синим показаны крупные маршруты, красным — узкие места мировой торговли, зелёным — важные торговые узлы.*

> [!IMPORTANT]
> На карте видно, что глобализация зависит не только от стран, но и от **географии**: от того, где находятся порты, каналы и проливы. Если один важный путь блокируется, проблемы могут почувствовать люди очень далеко от этого места.
...
Глобализация важна потому, что она помогает странам **специализироваться**. Одна страна может лучше делать электронику, другая — автомобили, третья — перевозки, четвёртая — программное обеспечение, пятая — добывать нефть или газ.
...
| интернет и цифровые сервисы | санкции и торговые барьеры |
...
У глобализации нет одной кнопки. Она работает сразу через несколько механизмов.

### 1. Международная торговля

Страны продают друг другу товары и услуги. Чем активнее торговля, тем сильнее они зависят друг от друга.

Например, [Азиатские тигры](./aziatskie_tigry.md) стали важной частью мировой экономики именно потому, что сумели встроиться в международную торговлю и производство.
...
Глобальная экономика работает не только на товарах, но и на платежах. Чтобы компании из разных стран могли быстро рассчитываться, нужны банки, валютные рынки и доверие к валютам. Поэтому важны [Доллар США](./dollar_ssha.md) и идея [резервной валюты](./rezervnaya_valyuta.md).
...
```mermaid
flowchart TD
    A[Глобализация] --> B[Торговля]
    A --> C[Транспорт]
    A --> D[Финансы]
    A --> E[Технологии]
    A --> F[Политические союзы]

    B --> B1[Экспорт и импорт]
    C --> C1[Корабли, самолёты, каналы]
    D --> D1[Валюты, банки, инвестиции]
    E --> E1[Интернет, чипы, платформы]
    F --> F1[ЕС, БРИКС и другие объединения]
```
...
|---|---|
| Больше выбора | в магазинах и онлайн больше товаров из разных стран |
...
| Новые рабочие места | в торговле, логистике, IT, промышленности, услугах |

| Проблемы | Что это значит простыми словами |
|---|---|
| Зависимость от дальних поставок | если где-то сбой, проблема ощущается в других странах |
| Неравномерная выгода | одни страны и компании выигрывают больше, чем другие |
| Давление на слабые отрасли | местные компании могут не выдержать конкуренцию |
| Риски для экологии | больше перевозок и производства — больше нагрузки на природу |
| Политические споры | торговля и финансы часто зависят от политики |

> [!WARNING]
> **Важно помнить:** глобализация сама по себе не «хорошая» и не «плохая». Это инструмент. Он может помогать развитию, а может делать мир более уязвимым к большим сбоям.
...
![Мировая торговля как доля мирового ВВП, 1970–2024](../images/globalizatsiya_trade_world_1970_2024.png)
...
Из таблицы видно: в целом торговая взаимосвязь мира за последние десятилетия стала значительно сильнее, хотя во время кризисов и потрясений бывают откаты.

### 2. Интернет как ускоритель глобализации

![Пользователи интернета в мире, 2005–2025](../images/globalizatsiya_internet_world_2005_2025.png)
...
![Мировые авиапассажиры, 1970–2023](../images/globalizatsiya_air_world_1970_2023.png)
...
### 4. Контейнерный трафик

![Мировой контейнерный портовый трафик, 2000–2022](../images/globalizatsiya_container_world_2000_2022.png)
...
Представьте обычный смартфон.

- идея и часть программ — из одной страны;
- чипы — из другой;
- экран — из третьей;
- редкие материалы приехали ещё из нескольких стран;
...
- почему телефон, приставка или видеокарта могут внезапно подорожать;
- почему задержка в одном канале или проливе может сказаться на ценах в магазине;
...
Если понимать глобализацию, легче понять и новости, и цены, и то, почему события далеко от тебя иногда влияют на твою повседневную жизнь.
...
> [!TIP]
> **Интересный факт:**  
> Даже если ты покупаешь вещь «одной марки», внутри неё часто спрятана работа десятков стран. Современный товар очень редко является результатом труда только одной экономики.

<a id="main"></a>
## Главное

Глобализация — это усиление связей между странами через торговлю, транспорт, деньги, технологии и культуру. Она помогает миру производить больше, быстрее обмениваться идеями и связывать рынки между собой. Но у неё есть и слабые места: чем сильнее связь, тем сильнее общий сбой, если ломается важный маршрут, растёт конфликт или останавливается поставка.
...
***Использованные нейросети и ресурсы:*** *ChatGPT 5.4; World Bank; ITU; ICAO; OECD; UNCTAD; Britannica; Natural Earth*
